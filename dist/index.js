 ...
 */
async function checkFolderWithoutRemove(folder) {
    const isExists = await fs__default['default'].pathExistsSync(folder);
    if (!isExists) {
        await fs__default['default'].mkdirsSync(folder);
    }
}
/**
...
async function writeEnums(folder, enums, templates) {
    ...
    await fs__default['default'].writeFileSync(folder + '/enum.ts', enumsData);
}
/**
...
async function writePermissions(permissions, outputPath, templates) {
    ...
    await fs__default['default'].writeFileSync(fileName, permissionTemplate);
}
...
async function generateEnums({ enums, outputPath, formatterFn, permissionKey, }) {
    const folder = path__namespace.resolve(process.cwd(), outputPath);
...
exports.addEnumTail = addEnumTail;
exports.codeSplitTransfer = codeSplitTransfer;
exports.generateEnums = generateEnums;
exports.generatorEnums = generatorEnums;
